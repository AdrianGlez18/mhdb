...
const displayedTitle = title.length > 30 ? title.slice(0, 30) + "..." : title
    ...
    return (
        <div className="py-3 w-full bg-gray-300 dark:bg-dark-700 border-white rounded-xl flex flex-col gap-2 items-center text-center justify-between">
            ...
            <div className="flex gap-3 items-center">
                <Link href={`/discover/books/${id}/add`} className="card-button w-full bg-green-300 p-3" title='Add to collection'>
                    <img src="/assets/icons/add-list.png" alt="Add to collection" aria-label='Add to collection' height={20} width={20} />

                </Link>
                <Link href={`/discover/books/${id}`} className="card-button w-full bg-green-300 p-3" title='View Details'>
