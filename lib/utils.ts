...
export const getTMDBTrendingList = async (contentType: string, page: number) => {
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/trending/${contentType}/week?api_key=85458e7043880e981cba6a2d69895ea8&language=en-US&page=${page.toString()}`,
...
export const getTMDBTrailer = async (content: any) => {
  ...
  const res = await fetch(
    `https://api.themoviedb.org/3/${content.media_type}/${content.id}/videos?api_key=85458e7043880e981cba6a2d69895ea8`,
    { next: { revalidate: 10000 } }
  );
  const data = await res.json();
  if (!res.ok) {
